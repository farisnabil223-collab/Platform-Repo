  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      if (response.data?.accessToken) {
        // Cache token locally so that follow-up /users/me query works
    ...
    <main className="min-h-screen bg-background text-foreground flex items-center justify-center p-6 select-none font-sans transition-colors">
      <Card className="max-w-md w-full bg-card border-border text-card-foreground shadow-2xl">
        <CardHeader className="p-6 text-center">
          ...
          <CardTitle className="text-card-foreground text-xl font-bold font-heading">Student Portal Login</CardTitle>
          <CardDescription className="text-muted-foreground text-xs mt-1">
            Access virtual classrooms and assignments.
          </CardDescription>
        </CardHeader>
