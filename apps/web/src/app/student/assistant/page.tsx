...
export default function StudentAIAssistantPage() {
  const [messages, setMessages] = React.useState<any[]>([
    { id: '1', sender: 'ai', text: 'Hello! I am your AI learning tutor. How can I assist you with your classes, assignments, or study plans today?' },
  ]);
  ...
  React.useEffect(() => {
    ...
    api.post('/v1/ai/copilot/conversations', { title: 'Student Chat Session' })
      .then((res: any) => {
  ...
  const shortcuts = [
    { label: 'Simplify Derivatives', prompt: 'Explain Calculus derivatives rules simply.' },
    { label: 'Study Plan', prompt: 'Draft a weekly physics study plan.' },
  ];

  return (
    <PortalLayout
      role="STUDENT"
      pageTitle="AI Student Tutor Workspace"
      pageDescription="Simplify complex topics, request step-by-step calculus hints, or organize exam checklists."
    >
        ...
        <div className="lg:col-span-3 border border-border/60 bg-card rounded-xl flex flex-col h-full overflow-hidden">
          ...
          >
            <input
              ...
              placeholder="Send a prompt to your tutor..."
              className="flex-1 bg-muted/20 border border-input rounded-lg px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-primary focus:bg-background text-white transition-all"
            />
